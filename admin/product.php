...
$query='';
if($_SESSION['ADMIN_TYPE']=='Admin' || $_SESSION['ADMIN_TYPE']=='Employee'){
...
$mysqluery=mysql_query($query);
 ...
 if(isset($_GET['searchkeyowords'])){
$searchid=$_GET['searchkeyowords'];
$query="select p.status,p.title,p.category_id,p.sub_category_id,p.price,p.pid,p.sub_sub_category_id,p.discount_price,p.date from product_details p inner join category c on c.cid=p.category_id  inner join sub_category sc on sc.scid=p.sub_category_id  where (p.title LIKE '%".$searchid."%' OR c.category_name LIKE '%".$searchid."%' OR sc.category_name LIKE '%".$searchid."%') and p.vendor_id='$userid' order by p.pid desc LIMIT ".$pageLimit." , ".$setLimit;

$mysqluery=mysql_query($query);
}else{
$query="select *from product_details where vendor_id='$userid' order by pid desc LIMIT ".$pageLimit." , ".$setLimit;
$mysqluery=mysql_query($query);
}   
}
...
<a href="#" class="btn btn-primary" value="Search" onclick="return search_result()">Search</a>
					   ...
					   ?>
                        <tr>
                          <td>
                              <?php echo $j++; ?>
                              <?php if($_SESSION['ADMIN_TYPE']=='Admin'){ ?>
                              <span> &nbsp;<input type="checkbox" style="display: inherit;margin: -17px 33px 0px;" name="checked_id[]" class="checkbox" onclick="check_checkboxes()" value="<?php echo $row['pid']; ?>"/></span>
                              <?php } ?>
                          </td>
	 ...
	 <a href="product.php?action=subsubcategory&catid=<?php echo $row['pid']; ?>"  class="btn btn-primary" title="Edit"><span class="glyphicon glyphicon-edit"></span></a>		 <?php } ?>
			&nbsp;&nbsp; 	
		  <?php if($_SESSION['ADMIN_TYPE']=='Admin'){ if((isset($_GET['page'])) && (isset($_GET['searchkeyowords']))) { ?>		 
			  ...
			  <?php }else{ ?>
			   <a href="post.php?action=deletemyproducts&delete=<?php echo $row['pid']; ?>" onclick="return confirm('are you sure to This Remove Records !'); " class="btn btn-danger" title="Delete" ><span class="glyphicon glyphicon-trash"></span></a>
                  ...
                  <?php } ?>
					...
					<?php
                                        if($_SESSION['ADMIN_TYPE']=='Admin' || $_SESSION['ADMIN_TYPE']=='Employee'){
                                        if(isset($_GET['searchkeyowords'])){ echo productsearchings($setLimit,$page,$_GET['searchkeyowords']);
					 } else{  echo productpagepaging($setLimit,$page,$_SESSION['ADMIN_ID']);  } } else {
                                             if(isset($_GET['searchkeyowords'])){ echo productsearchings_vendors($setLimit,$page,$_GET['searchkeyowords'],$_SESSION['ADMIN_ID']);
					 } else{  
                                        echo productpagepaging_employee($setLimit,$page,$_SESSION['ADMIN_ID']);  } }
                                             ?>
		</div></div>
		</div>
		<?php } ?>
